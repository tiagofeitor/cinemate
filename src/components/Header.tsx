...
export const Header = () => {
  ...
  const inactiveClass = "text-lg block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700";

  const [hidden, setHidden] = useState<Boolean>(true) ;
  
    /* dark mode local storage and retrieval */
  const StorageDarkMode = localStorage.getItem("darkMode") || "";

  const [darkMode, setDarkMode] = useState(StorageDarkMode ? JSON.parse(StorageDarkMode) : false);
  /*dark mode hook to update and store darkMode toggle*/
  useEffect(()=>
    { 
    ...
    [darkMode]);
    //handle submit search movie and navigate to Search component
    ...
    <header >
      ...
      <nav className="bg-white border-b-2 border-gray-200 dark:bg-gray-800 dark:border-b-1 dark:border-gray-900">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <Link to="/" className="flex items-center">
              <img src={Logo} className="h-8 mr-2 my-1" alt="Cinemate Logo" />
              <span className="self-center text-3xl font-semibold whitespace-nowrap dark:text-white">Cinemate</span>
          </Link>

          {/* MOBILE NAV LINK*/}
          <div id="mobile-nav" className="flex margin-t-2 md:order-2">
